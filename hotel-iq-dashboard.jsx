...
export default function HotelIQ() {
  ...
  // theme management (dark/light) persisted in localStorage
    ...
    <div style={{
          ...
          <div style={{ position: "relative" }}>
            <button onClick={() => setShowNotif(v => !v)} style={{
              ...
              {notifications.length > 0 && (
                <span style={{
                  ...
                  background: "#F87171", borderRadius: "50%", fontSize: 10,
            ...
            {showNotif && (
              <div style={{
                ...
                width: 260, background: cardBg, border: cardBorder,
                borderRadius: 8, padding: 12,
                boxShadow: "0 8px 20px rgba(0,0,0,0.5)", zIndex: 150
              }}>
                {notifications.map(n => (
                  <div key={n.id} style={{ padding: "8px 0", borderBottom: "1px solid rgba(255,255,255,0.1)", fontSize: 13, color: isDark ? "#eee" : "#111" }}>
                    <div>{n.text}</div>
                    <div style={{ fontSize: 10, color: "#888" }}>{n.time}</div>
                  </div>
                ))}
                {notifications.length === 0 && <div style={{ textAlign: "center", color: "#888", fontSize: 12 }}>No notifications</div>}
              </div>
            )}
          </div>
        ...
        {activeTab === "settings" && (
          <div style={{ display: "flex", flexDirection: "column", gap: 32 }}>
            <div>
              ...
              <p style={{ color: "#555", margin: "6px 0 0", fontSize: 14 }}>Configure your preferences</p>
            </div>
            ...
            <div style={{ background: cardBg, border: cardBorder, borderRadius: 16, padding: 28 }}>
              <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
                ...
                <button onClick={toggleTheme} style={{
                  background: "transparent", border: `1px solid ${isDark ? "#555" : "#444"}`,
      ...
      <style>{`
        ...
        * { box-sizing: border-box; }
        ...
        ::-webkit-scrollbar-thumb { background: ${isDark ? "#2A2A2A" : "#888"}; border-radius: 2px; }
